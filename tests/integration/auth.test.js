...
describe('Auth routes', () => {
  describe('POST /v1/auth/register', () => {
    test('should return 404 - self-registration is disabled, accounts are admin-created only', async () => {
      const newUser = {
        name: 'fake name',
      ...
      await request(app).post('/v1/auth/register').send(newUser).expect(httpStatus.NOT_FOUND);
    });
  });

  describe('POST /v1/auth/login', () => {
    test('should return 200 and login user if phone number and password match', async () => {
    ...
    test('should return 401 error if there are no users with that phone number', async () => {
      const loginCredentials = {
        phoneNumber: userOne.phoneNumber,
        password: userOne.password,
      };
      ...
      expect(res.body).toEqual({ code: httpStatus.UNAUTHORIZED, message: 'Incorrect phone number or password' });
    });
  ...
  describe('POST /v1/auth/forgot-password', () => {
    ...
    test('should return 204 and send reset password email to the user', async () => {
      ...
      await request(app).post('/v1/auth/forgot-password').send({ email: agent.email }).expect(httpStatus.NO_CONTENT);

      expect(sendResetPasswordEmailSpy).toHaveBeenCalledWith(agent.email, expect.any(String));
      const resetPasswordToken = sendResetPasswordEmailSpy.mock.calls[0][1];
      const dbResetPasswordTokenDoc = await Token.findOne({ token: resetPasswordToken, user: agent._id });
    ...
    test('should return 404 if email does not belong to any user', async () => {
      await request(app).post('/v1/auth/forgot-password').send({ email: agent.email }).expect(httpStatus.NOT_FOUND);
    });
  });
  ...
  describe('POST /v1/auth/reset-password', () => {
    test('should return 204 and reset the password', async () => {
      ...
      const resetPasswordToken = tokenService.generateToken(agent._id, expires, tokenTypes.RESET_PASSWORD);
      await tokenService.saveToken(resetPasswordToken, agent._id, expires, tokenTypes.RESET_PASSWORD);
      ...
      const dbUser = await User.findById(agent._id);
      const isPasswordMatch = await bcrypt.compare('password2', dbUser.password);
      expect(isPasswordMatch).toBe(true);

      const dbResetPasswordTokenCount = await Token.countDocuments({ user: agent._id, type: tokenTypes.RESET_PASSWORD });
    ...
    test('should return 401 if reset password token is blacklisted', async () => {
      ...
      const expires = moment().add(config.jwt.resetPasswordExpirationMinutes, 'minutes');
      const resetPasswordToken = tokenService.generateToken(agent._id, expires, tokenTypes.RESET_PASSWORD);
      await tokenService.saveToken(resetPasswordToken, agent._id, expires, tokenTypes.RESET_PASSWORD, true);
  ...
  describe('POST /v1/auth/send-verification-email', () => {
    ...
    test('should return 204 and send verification email to the user', async () => {
      ...
      expect(sendVerificationEmailSpy).toHaveBeenCalledWith(agent.email, expect.any(String));
      const verifyEmailToken = sendVerificationEmailSpy.mock.calls[0][1];
      const dbVerifyEmailToken = await Token.findOne({ token: verifyEmailToken, user: agent._id });
...
describe('Auth middleware', () => {
  ...
  test('should call next with no errors if user has required rights', async () => {
    ...
    await auth(...roleRights.get('super_admin'))(req, httpMocks.createResponse(), next);
